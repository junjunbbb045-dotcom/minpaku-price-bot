...
// 凡例は3セル1組（真ん中のセルに中央揃えでラベル）
function legendTriple(label, bg) {
  return [blank(bg), cell(label, { bg, bold: true, center: true }), blank(bg)];
}
...
export async function updateSheetsDashboard(data, priceLookup) {
  ...
  const requests = [
    ...
    // 書式を全面リセットしてから書き込む（レイアウト変更で残る古い色を消す）
    {
      repeatCell: {
    ...
    { updateDimensionProperties: { range: columnRange(sheetId, 0, 1), properties: { pixelSize: 140 }, fields: 'pixelSize' } },
    { updateDimensionProperties: { range: columnRange(sheetId, 1, 2), properties: { pixelSize: 60 }, fields: 'pixelSize' } },
    { updateDimensionProperties: { range: columnRange(sheetId, 2, 4), properties: { pixelSize: 68 }, fields: 'pixelSize' } },
    {
      updateDimensionProperties: {
    ...
    },
    {
      setBasicFilter: {
        filter: {
          range: { sheetId, startRowIndex: HEADER_ROWS - 1, endRowIndex: dataRowCount, startColumnIndex: 0, endColumnIndex: TOTAL_COLUMNS },
        },
      },
    },
