...
function priceCellHtml(value, note) {
  if (value == null) {
    const color = note === '満室' ? COLORS.soldOutText : COLORS.noDataText;
    return `<span style="color:${color};">${escapeHtml(note ?? 'データなし')}</span>`;
  }
  return yen(value);
}
...
function rowHtml(r) {
  ...
  return `
      <tr style="background:${rowBg};">
        ...
        <td style="padding:10px 12px;border-bottom:1px solid #eeeeee;text-align:right;font-size:13px;">${priceCellHtml(r.weekdayAverage, r.weekdayNote)}</td>
        <td style="padding:10px 12px;border-bottom:1px solid #eeeeee;text-align:right;font-size:13px;">${priceCellHtml(r.weekendHolidayAverage, r.weekendHolidayNote)}</td>
        <td style="padding:10px 12px;border-bottom:1px solid #eeeeee;text-align:center;font-size:12px;color:#777777;">${r.fetchedDays}</td>
        <td style="padding:10px 12px;border-bottom:1px solid #eeeeee;text-align:center;font-size:12px;color:#777777;">${r.soldOutDays}</td>
      </tr>`;
}
  ...
  return `
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;margin-bottom:8px;">
        <tr style="background:${hBg};">
          ...
          <th style="padding:8px 12px;color:${COLORS.headerText};text-align:center;font-size:12px;">価格取得日数</th>
          <th style="padding:8px 12px;color:${COLORS.headerText};text-align:center;font-size:12px;">満室日数</th>
        </tr>${bodyRows}
      </table>`;
}
...
function sectionHtml(week, weeklyStats) {
  ...
  return `
    <tr><td style="padding:24px 24px 8px;">
      <h2 style="margin:0 0 4px;font-size:15px;color:#1a3a5c;">
        ${escapeHtml(week.label)}: ${week.weekStart} 〜 ${week.weekEnd}
      </h2>
      ${groupTables}
    </td></tr>`;
}
    ...
    <tr><td align="center" style="padding:24px 12px;">
      <table width="640" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:10px;overflow:hidden;border:1px solid #e2e2e2;">
        ...
        <tr><td style="padding:8px 24px 24px;color:#999999;font-size:11px;">
          平日＝日〜金（連休中日除く）／土曜・連休＝土曜・連休中日・お盆・年末年始・GW・シルバーウィーク／平均は空室日すべての価格（取得後${data.priceMaxAgeDays ?? 5}日以内）から算出し満室日は除外／宿泊人数: 大人2名（1泊）
        </td></tr>
      </table>
    </td></tr>
  </table>
